...
Ext.extend(Ext.grid.GridFilters, Ext.util.Observable, {
	/**
	 * @cfg {Integer} updateBuffer Number of milisecond to defer store
	...
	paramPrefix : 'filter',
	/**
	 * @cfg {String} fitlerCls The css class to be applied to column
	 *      headers that active filters. Defaults to 'ux-filterd-column'
	 */
	filterCls : 'ux-filtered-column',
	/**
	 * @cfg {Boolean} local True to use Ext.data.Store filter functions
	 *      instead of server side filtering.
	 */
	local : false,
	/**
	 * @cfg {Boolean} autoReload True to automagicly reload the
	 *      datasource when a filter change happens.
	 */
	autoReload : true,
	/**
	 * @cfg {String} stateId Name of the Ext.data.Store value to be used
	 *      to store state information.
	 */
	stateId : undefined,
	/**
	 * @cfg {Boolean} showMenu True to show the filter menus
	 */
	showMenu : true,
	...
	init : function(grid) {
		if (grid instanceof Ext.grid.GridPanel) {
			...
			if (this.local) {
				this.store.on('load', this.filterOnLoad, this);
			} else {
    ...
    reconfigure: function(g) {
        ...
        var toolbar = g.getBottomToolbar();
        
        this.grid.un('render', this.onRender);
        this.grid.un('beforestaterestore', this.applyState);
        this.grid.un('beforestatesave', this.saveState);
        
        
        this.store = this.grid.getStore();
        if (this.local) {
            this.store.un('load', this.filterOnLoad);
            this.store.on('load', this.filterOnLoad, this)
        } else {
        ...
        this.grid.on("beforestaterestore", this.applyState, this);
        this.grid.on("beforestatesave", this.saveState, this);
    },
    
    filterOnLoad: function(store) {
	...
	addFilter : function(config) {
		var filter = config.menu ? config : new (this
				.getFilterClass(config.type))(config);
	 ...
	 * query parameteres. The default function will produce a query
	 * string of the form:
	 ...
	 * 
	 * @param {String}
	 *            type The type of filter to load.
	 * 
	 * @return {Class}
	 */
