...
Ext.menu.RangeMenu = function(config){
	...
	var fields = this.fields = Ext.applyIf(this.fields || {}, {
		...
		'neq': new Ext.menu.EditableItem({
			icon:   this.icons.neq, 
			editor: new cls(typeof cfg == "object" ? cfg.eq || '' : cfg)
    })
	});
	this.add(fields.gt, fields.lt, '-', fields.eq, fields.neq);
...
Ext.extend(Ext.menu.RangeMenu, Ext.menu.Menu, {
	...
	icons: {
		...
		neq: '/img/small_icons/not_equals.png'
  },
    ...
    onKeyUp: function(event, input, notSure, field) {
    	...
    	switch (field) {
    	    case this.fields.eq:
                ...
                this.fields.neq.setValue(null);
                break;
            case this.fields.neq:
                this.fields.gt.setValue(null);
                this.fields.lt.setValue(null);
