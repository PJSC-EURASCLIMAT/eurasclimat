...
Ext.extend(Ext.ux.Andrie.pPageSize, Ext.util.Observable, {
	...
	setPageSize: function(value, forced) {
		...
		if (value == pt.pageSize) {
			return;
		} else if (value < pt.pageSize && !this.forceLoad) {
			pt.pageSize = value;
	...
	onRender: function() {
		...
		var comboConfig = {
            ...
            applyState: function(state) {
            	self.pagingToolbar.pageSize = state;
            }
        }; 
        
		if (this.stateId) {
			comboConfig.stateEvents = ['select'];
			comboConfig.stateful = true;
			comboConfig.stateId = this.stateId + '.pPageSize';
		}
        
        this.combo = Ext.ComponentMgr.create(Ext.applyIf(this.comboCfg||{}, comboConfig));
        
		this.combo.on('select', this.setPageSize, this);
		this.combo.on('render', function(c) {
			c.getEl().parent().parent().setWidth(c.width + c.offsetWidth + 'px');
		});
